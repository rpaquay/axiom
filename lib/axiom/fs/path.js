...
/**
 * Returns an absolute path for pathSpec, using pwd as prefix if necessary.
 * @param {string} pwd
...
Path.abs = function(pwd, pathSpec) {
  ...
  return pwdPath.combine(pathSpec);
};
...
Path.prototype.combine = function(relSpec) {
  if (relSpec.substr(0, 2) == './')
    relSpec = relSpec.substr(2);

  // If this has no elements, don't add extra separator.
  if (this.elements.length == 0) {
    // Example: 'fs:/' + 'foo' => 'fs:/foo'
    return new Path(this.spec + relSpec);
  }
  else {
    // Example: 'fs:/bar' + 'foo' => 'fs:/bar/foo'
