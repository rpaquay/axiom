...
JsFileSystem.prototype.mkdir = function(path) {
  ...
  if (!rv.entry.hasMode('D')) {
    return Promise.reject(
        new AxiomError.TypeMismatch('dir', parentPath.originalSpec));
  }
...
JsFileSystem.prototype.alias = function(pathFrom, pathTo) {
  ...
  if (!resolveFrom.isFinal) {
    ...
    // doesn't exist.
    return Promise.reject(
        new AxiomError.NotFound('path', pathFrom.originalSpec));
  }
  ...
  if (resolveTo.isFinal) {
    ...
    return Promise.reject(new AxiomError.NotFound('path', pathTo.originalSpec));
  }
...
JsFileSystem.prototype.createExecuteContext = function(exePath, stdio, arg) {
  ...
  if (!rv.isFinal) {
    ...
    return Promise.reject(
        new AxiomError.NotFound('path', exePath.originalSpec));
  }
